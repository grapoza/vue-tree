...
|:------------------|:------------------------------------|:--------------------------------------------------------------------------------------------------------------------------|:----------------------------------|:---------|
| modelValue        | Array                               | The data model containing a tree structure of node data; used with `v-model`                                              |                                   | Yes      |
...
| filterMethod      | TreeViewFilterMethod                | A boolean function against which nodes are evaluated to see if they should be included in the tree                        | `null` (no filtering)             |          |
| loadNodesAsync    | TreeViewLoadNodesAsyncMethod        | A function that is called on mount to asynchronously load the root data model (use with a `v-model` of `[]`)              | `null`                            |          |
| modelDefaults     | TreeViewNodeMetaModelDefaultsMethod | An function that returns the default metadata for a given data node from the `modelValue` (see [Model Data](#model-data)) | `() => {}`                        |          |
| selectionMode     | SelectionMode                       | How selection should operate (see [Selection Mode](#selection-mode))                                                      | `null` (cannot select nodes)      |          |
| skinClass         | String                              | A class name to apply to the tree that specifies a skin to use (see [Skins](#skins))                                      | `"grtv-default-skin"`             |          |

## Selection Mode

The `selectionMode` property defines how nodes should be selected within the tree view. Only nodes with the `selectable` model property set to `true` can be selected. Allowed values follow the enum:
...
Model data can be loaded either synchronously through the `v-model` or asynchronously through the `loadNodesAsync` property. If both are specified then the data from `v-model` is overwritten when the `loadNodesAsync` function returns data, so a `v-model` of `[]` is the appropriate value to use when loading the root nodes asynchronously.

The data passed to the tree view's `v-model` or returned from `loadNodesAsync` should be an array of nodes, where each node should have:
...
* Optional: A property containing any subnodes (by default the node looks for a property named `children`); an empty array is required if the children will be loaded asynchronously
* Optional: Any other data you want on your node

Meta data about a node in the tree is created when the node is initialized in the tree view, and can be customized using the `modelDefaults` property. The `modelDefaults` should be a method that takes the data for the node from the `v-model` and returns an object specifying any default tree node state related to that node. That returned value will be merged with the defaults to create the node meta data used by the tree view.

The meta data for the entire tree is exposed via a `metaModel`, and can be manipulated to programmatically change the tree state if needed, _e.g._, to set a checkbox's checked state or a node's selectable state.

Some example node data, as passed to `v-model`:
...
And a function to use for `modelDefaults for those nodes:
...
The properties below can be specified for each node. Note that `id`, `label`, and `children` are the default properties nodes look for to get data, but the names of the properties can be overridden using the `modelDefaults`.
...
The `modelDefaults` property's return value contains any data about the node's capabilities and its initial state. This keeps the tree-specific data separate from the data of the model itself. This makes it more convenient to drop data into the tree view as-is, potentially with a `modelDefaults` specified on the tree view to define common values used in every node. The following table lists the properties that can be part of the value returned from `modelDefaults`.
...
| dataTransferEffectAllowed | EffectAllowed | One of the allowed [effectAllowed](https://developer.mozilla.org/en-US/docs/Web/API/DataTransfer/effectAllowed) values (`copy`, `move`, `copyMove`, or `none` are honored) | `copyMove` | |
...
| addChildCallback     | TreeViewAddChildCallbackMethod | An async function that resolves to a new node model (called by the add button). The function can take one argument, the model of the parent node. It should return the model of the new child node. | `null` | |
| loadChildrenAsync    | TreeViewLoadChildNodesAsyncMethod | An async function that resolves to a node's children (called when the parent is expanded). The function can take one argument, the model of the parent node. It should return the models of the children nodes. | `null` | |
| deleteNodeCallback   | TreeViewDeleteNodeCallbackMethod | An async function that resolves to a boolean (called by the delete button). The function can take one argument, the model to delete. It should return a boolean indicating whether to delete the node. | `null` | |
...
| metaModel              | An object containing the meta data for the tree nodes. Its structure mirrors the `v-model` data. | N/A        | N/A        |
...
It's often helpful to be able to make adjustments to the markup or styles for the tree. You can provide an object from the `modelDefaults` using the `customizations` property to set a customization affecting all or some nodes.
...
| text         | Replaces the span used for non-input content                | metaModel - The TreeViewNode's metaModel                                                           |
|              |                                                             | customClasses - Any custom classes specified in `treeNodeSpec.customizations`                      |
| checkbox     | Replaces the label and content used for checkboxes          | metaModel - The TreeViewNode's metaModel                                                           |
|              |                                                             | customClasses - Any custom classes specified in `treeNodeSpec.customizations`                      |
...
| radio        | Replaces the label and content used for radio buttons       | metaModel - The TreeViewNode's metaModel                                                           |
|              |                                                             | customClasses - Any custom classes specified in `treeNodeSpec.customizations`                      |
|              |                                                             | inputId - The ID for the input (as generated by the TreeViewNode)                                  |
|              |                                                             | radioChangeHandler - The handler for radio button change events. You should fire this on `change`. |
| loading      | Replaces the span used when loading children asynchronously | metaModel - The TreeViewNode's metaModel                                                           |
|              |                                                             | customClasses - Any custom classes specified in `treeNodeSpec.customizations`                      |
| expander     | Replaces the button and spacer used for expanding nodes     | metaModel - The TreeViewNode's metaModel                                                           |
|              |                                                             | customClasses - Any custom classes specified in `treeNodeSpec.customizations`                      |
|              |                                                             | expanderId - The ID for the expander button as generated by the TreeViewNode)                      |
|              |                                                             | canExpand - The TreeViewNode's computed boolean indicating whether the node can expand             |
|              |                                                             | toggleNodeExpanded - A method to toggle the expanded state of the node                             |

Example usage:

```html
<TreeView id="customtree" v-model="model">
  <template #text="{ metaModel, customClasses }">
    <!-- The TreeViewNode's metaModel is available, and built-in classes and overrides are available -->
    <em :title="metaModel.title"
              class="grtvn-self-text"
              :class="customClasses.treeViewNodeSelfText">
          {{ metaModel.data[metaModel.labelProperty] }}
    </em>
  </template>
  ...
  <template #checkbox="{ metaModel, customClasses, inputId, checkboxChangeHandler }">
    <label :for="inputId"
      ...
      <em>{{ "Slotted Content for " + metaModel.data[metaModel.labelProperty] }}</em>
    </label>
  </template>
</TreeView>
```
...
Child nodes can be loaded asynchronously by providing a method to the `loadChildrenAsync` property of the model returned by `modelDefaults`. The method will be called when the node is expanded, and the property containing the node's children will be _overwritten_ with the results of the async call. For example, the following could be used as `modelDefaults` that would have every node load children when it was expanded.
...
The method may accept one argument, which will be the meta data of the node that has been expanded. It should resolve to an array of child node models.

The load method is called once, and after that the children are part of the model and are not reloaded.

## Drag and Drop

A user can drag and drop an individual TreeViewNode. A drag only affects the node where the dragging starts, and has nothing to do with any node selection within the tree. To make a node draggable, the node's `draggable` must be `true` in its meta data. To make a node accept drops, the `allowDrop` property must be `true` in the node's meta data. Both Move and Copy operations are supported. To copy in most browsers hold down the `Ctrl` key while dragging.
...
The tree view uses a roving tab index to maintain focusability in the tree. A node's meta model can specify a `focusable` property of `true` in order for that node to be used as the initial target of keyboard focus within the tree view. If multiple nodes specify this then only the first will have `focusable` set to `true` once the tree view is intialized. If no models have it specified then the first selected node in the tree view is given a `focusable` of `true`. If there are no selected nodes then the first node in the tree view is given a `focusable` of `true`.
...
If a `filterMethod` property is provided to the treeView then each node will get evaulated against that method, any node for which the function returns `true` or for which a subnode returns `true` will show in the tree view and get included in things like keyboard navigation or focusing. The method takes one parameter, which is the node. For example, to only show nodes with a description including "steve" you could use the method:

`(node) => node.label.toLowerCase().includes('steve')`