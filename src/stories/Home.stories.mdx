...
|:------------------|:---------|:---------------------------------------------------------------------------------------------------|:----------------------------------|:---------|
| modelValue        | Array    | The data model containing [model data](#model-data); used with `v-model`                           | `[]`                              |          |
...
Model data can be loaded either synchronously through the `modelValue` property or asynchronously through the `loadNodesAsync` property. If both are specified then the data from `modelValue` is overwritten when the `loadNodesAsync` function returns data.

The data passed to the tree view's `modelValue` prop or returned from `loadNodesAsync` should be an array of nodes, where each node should have:
...
The `treeNodeSpec` of the objects in the data model passed to the tree view's `modelValue` property will be updated by the tree view nodes themselves on creation to include missing properties.
...
|              |                                                             | customClasses - Any custom classes specified in `treeNodeSpec.customizations`                      |
| expander     | Replaces the button and spacer used for expandign nodees    | metaModel - The TreeViewNode's metaModel                                                           |
|              |                                                             | customClasses - Any custom classes specified in `treeNodeSpec.customizations`                      |
